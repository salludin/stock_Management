...
class Users extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['title'] = $this->lang->line('let_staff_mg');
		$data['all_countries'] = $this->Xin_model->get_countries();
		$data['all_user_roles'] = $this->Roles_model->all_user_roles();
		$data['breadcrumbs'] = $this->lang->line('let_staff_mg');
		$data['path_url'] = 'users';
    ...
    public function users_list()
     {
		...
		$data = array();
          ...
          foreach($users->result() as $r) {
			...
			if($r->user_id!=1):
				$opt = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->user_id . '"><i class="fa fa-trash-o"></i></button></span>';
			else:
				$opt = '';	
			endif;
			
		   $data[] = array(
				'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-user_id="'. $r->user_id . '"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-user_id="'. $r->user_id . '"><i class="fa fa-eye"></i></button></span>'.$opt.'',
				$full_name,
				$r->email,
				$r->username,
				$r->contact_number,
	...
	public function add_user() {
		...
		'fixed_header' => 'fixed_layout_hrsale',
		...
		if ($result == TRUE) {
			...
			$message = '
				...
				<img src="'.$logo.'" title="'.$cinfo[0]->company_name.'"><br>'.str_replace(array("{var site_name}","{var user_name}","{var full_name}","{var email}","{var password}"),array($cinfo[0]->company_name,$this->input->post('username'),$full_name,$this->input->post('email'),$this->input->post('password')),htmlspecialchars_decode(stripslashes($template[0]->message))).'</div>';
	...
	public function update_profile() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_profile_updated_user');
		} else {
			$Return['error'] = $Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
	...
	public function delete() {
		...
		if($this->input->post('is_ajax')==2) {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('xin_success_delete_user');
			} else {
				$Return['error'] = $Return['error'] = $this->lang->line('xin_error_msg');
			}
			$this->output($Return);
