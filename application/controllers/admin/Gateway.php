...
class Gateway extends MY_Controller {
	...
	public function paypal_process($param1 = '',$param2='')
    {
		...
		if ($param1 == '') {
            ...
            $this->paypal->add_field('notify_url', site_url() . 'admin/gateway/paypal_process/paypal_ipn');
            $this->paypal->add_field('cancel_return', site_url() . 'admin/gateway/paypal_process/paypal_cancel/'.$invoice->invoice_id);
            $this->paypal->add_field('return', site_url() . 'admin/gateway/paypal_process/paypal_success/'.$invoice->invoice_id);
        ...
        if ($param1 == 'paypal_cancel') {
			...
			redirect('admin/orders/view/'.$param2, 'refresh');
        }
        ...
        if ($param1 == 'paypal_success') {
			...
			'payer_payee_id' => $invoice[0]->customer_id,
			'payment_method_id' => 1,
			...
			$iresult = $this->Transactions_model->add_transactions($ivdata);
			if ($iresult == TRUE) {			
				$data = array(
					'status' => 1,
				);
	...
	public function stripe_charge($id){
       ...
       if($this->input->post('stripeToken')) {
			...
			//get customer email
			$customer_email = $this->Invoices_model->client_email_for_invoice($invoice_id);
			...
			$this->session->set_flashdata('success', 'Thank you payment has done sucessfully.');
			redirect('admin/orders/view/'.$id.'/', 'refresh');
